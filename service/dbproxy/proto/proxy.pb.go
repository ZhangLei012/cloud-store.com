...
type SingleAction struct {
	...
	Params []byte `protobuf:"bytes,2,opt,name=params,proto3" json:"params,omitempty"` //请求参数列表，json-code
}
...
type ReqExec struct {
	...
	Sequence    bool            `protobuf:"varint,1,opt,name=sequence,proto3" json:"sequence,omitempty"`       //是否按照给定顺序串行执行
	Transaction bool            `protobuf:"varint,2,opt,name=transaction,proto3" json:"transaction,omitempty"` //所有action是否在一个事务里执行
	ReturnType  int32           `protobuf:"varint,3,opt,name=returnType,proto3" json:"returnType,omitempty"`   //0表示每个sql函数的结果都返回；1表示只返回最后一个执行sql的结果（要求sequence）执行
...
func (x *ReqExec) GetReturnType() int32 {
	if x != nil {
		return x.ReturnType
	}
	return 0
...
var file_proxy_proto_rawDesc = []byte{
	...
	0x0a, 0x72, 0x65, 0x74, 0x75, 0x72, 0x6e, 0x54, 0x79, 0x70, 0x65, 0x18, 0x03, 0x20, 0x01, 0x28,
	0x05, 0x52, 0x0a, 0x72, 0x65, 0x74, 0x75, 0x72, 0x6e, 0x54, 0x79, 0x70, 0x65, 0x12, 0x3e, 0x0a,
